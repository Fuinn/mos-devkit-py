...
class TestConstraints(unittest.TestCase):
    ...
    def test_slack(self):
        ...
        self.assertTrue(isinstance(c1.slack, devkit.model.variable.VariableScalar))
        self.assertTrue(isinstance(c2.slack, devkit.model.variable.VariableScalar))
        self.assertTrue(isinstance(c3.slack, devkit.model.variable.VariableScalar))
    ...
    def test_equal_contruction(self):
        ...
        z = devkit.model.variable.VariableMatrix('z', shape=(2,3))
        ...
        c = x == r
        self.assertTrue(isinstance(c, devkit.model.constraint.ConstraintArray))
        for i in range(2):
            for j in range(3):
                cij = c[i,j]
                self.assertTrue(isinstance(cij, devkit.model.constraint.Constraint))
        ...
        for i in range(2):
            for j in range(3):
                ...
                self.assertTrue(isinstance(cij, devkit.model.constraint.Constraint))
                self.assertEqual(cij.op, '==')
                self.assertTrue(isinstance(cij.lhs, devkit.model.function.multiply))
                self.assertTrue(isinstance(cij.rhs, devkit.model.function.add))
    ...
    def test_bad_array_construction(self):
        ...
        x = devkit.model.VariableMatrix('x', np.random.randn(4,3))
        y = devkit.model.VariableScalar('y', 5)

        c0 = x == 1
        c1 = y == 0

        c = devkit.model.constraint.ConstraintArray(c0)
        c = devkit.model.constraint.ConstraintArray(c1)
        c = devkit.model.constraint.ConstraintArray([c1])
        
        self.assertRaises(TypeError, devkit.model.constraint.ConstraintArray, [x, y])
        self.assertRaises(TypeError, devkit.model.constraint.ConstraintArray, [c0, c1])
        self.assertRaises(TypeError, devkit.model.constraint.ConstraintArray, ['foo'])
    ...
    def test_flatten_to_list(self):
        ...
        for i in range(4):
            for j in range(3):
                self.assertTrue(isinstance(cfl[i*3+j], devkit.model.constraint.Constraint))
    ...
    def test_std_components(self):
        ...
        # Nonlinear ==
        c = 4*x + x*y + devkit.model.sin(y) == 1
        ...
        # Nonlinear <=
        c = 4*x + x*y + devkit.model.sin(y) <= 5 
        ...
        # Nonlinear >=
        c = 4*x + x*y + devkit.model.sin(y) >= 13.
