...
from ..solver import SolverError, SolverINLP
...
class Model(object):
    ...
    def __get_std_problem__(self, fast_evaluator=True):
        ...
        u_list = comp['u_list']
        l_list = comp['l_list']
        u = self.INF*np.ones(num_vars)
        l = -self.INF*np.ones(num_vars)
        ...
        if comp['phi_prop']['affine'] and all([prop['affine'] for prop in comp['prop_list']]):
            p.properties.append(Problem.PROP_CURV_LINEAR)
        else:
            p.properties.append(Problem.PROP_CURV_NONLINEAR)

        # Properties (var types)
        if np.any(p.P):
            p.properties.append(Problem.PROP_VAR_INTEGER)
        else:
            p.properties.append(Problem.PROP_VAR_CONTINUOUS)

        # Properties (problem type)
        if not comp['phi_prop']['a'].keys():
            p.properties.append(Problem.PROP_TYPE_FEASIBILITY)
        else:
            p.properties.append(Problem.PROP_TYPE_OPTIMIZATION)
